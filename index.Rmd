...
Currently we recommend that you cite either the website (https://exp.psy.gla.ac.uk/), the codebase on GitHub (https://github.com/debruine/experimentum) or this manual, following the appropriate citation format for your discipline.
...
read_csv("misc_files/author_contribs.csv") %>%
  arrange(surname) %>%
  unite(col = "name", c(first_name, surname), sep = " ") %>%
  mutate(`Author:` = paste0(name, ": "),
         ` ` = Contribution) %>%
  select(`Author:`, ` `) %>%
  knitr::kable()
```
